...
highlight All Locations Recorded

awk '!NF || !seen[$2]++'  speciesDetails.txt > locationsF.txt
awk '{print NR" ) "$2}' locationsF.txt > locationsFN.txt
cat locationsFN.txt
echo 'What Locations records do you want to send?'
read specificLocation
specVar=$(awk '$1 == '$specificLocation' {print $3}' locationsFN.txt)
echo $specVar
IFS=

case $specificLocation in
